...
- `Makefile` is the command source of truth. Current targets: `make test`, `make build`, `make fmt`, `make run-health`, `make run-customer-list`, `make run-invoice-import FILE=...`.
...
- Main entrypoint: `cmd/cli/main.go` wires the CLI.
...
- CLI commands must call shared `internal/app` services; connectors/infra must not bypass app services for core state changes.
- Config auto-loads `.env`; existing non-empty environment variables win. `BILLAR_DB_PATH` defaults to a per-user SQLite DB; `BILLAR_EXPORT_DIR` roots CLI PDF/file outputs.
