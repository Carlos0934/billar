...
func (localSessionService) StartLogin(context.Context) (app.LoginIntentDTO, error) {
...
func main() {
	...
	server := mcpconnector.NewServer(localSessionService{sessions: sessionStore}, app.NewCustomerService(sessionStore, infrasqlite.NewCustomerStore(store)), mcpconnector.NewIngressGuardFromConfig(cfg.AccessPolicy), logger)
